...
import { t as getAuth } from "./auth_CyDY6j-t.mjs";
import { i as eq } from "./drizzle-orm_BlbJG4Hn.mjs";
import { n as employers, o as users, r as jobs, t as candidateProfiles } from "./schema_BqQlk95h.mjs";
...
//#endregion
//#region src/pages/api/[...route].ts
var ____route__exports = /* @__PURE__ */ __exportAll({ ALL: () => ALL });
...
app.post("/auth/register", async (c) => {
	try {
		...
		if (await db.select().from(users).where(eq(users.email, email)).get()) return c.json({ error: "Email already in use" }, 400);
		...
		if (role === "EMPLOYER") await db.insert(employers).values({
			id: crypto.randomUUID(),
			userId,
			companyName: fullName
		});
		else await db.insert(candidateProfiles).values({
			id: crypto.randomUUID(),
			userId,
			fullName
		});
...
app.post("/auth/login", async (c) => {
	try {
		...
		const user = await getDb(c.env).select().from(users).where(eq(users.email, email)).get();
		if (!user || !user.hashedPassword) return c.json({ error: "Invalid email or password" }, 400);
		if (!(await hashPassword(password) === user.hashedPassword)) return c.json({ error: "Invalid email or password" }, 400);
		const lucia = getAuth(c.env);
...
app.put("/admin/jobs/:id/status", async (c) => {
	...
	await getDb(c.env).update(jobs).set({ status }).where(eq(jobs.id, id));
	return c.json({ success: true });
});
app.put("/admin/jobs/:id", async (c) => {
	...
	await getDb(c.env).update(jobs).set({
		title: data.title,
...
app.delete("/admin/jobs/:id", async (c) => {
	...
	const id = c.req.param("id");
	const db = getDb(c.env);
	const { applications } = await import("./schema_BqQlk95h.mjs").then((n) => n.i);
...
app.put("/admin/users/:id/role", async (c) => {
	...
	await getDb(c.env).update(users).set({ role }).where(eq(users.id, id));
	return c.json({ success: true });
});
app.delete("/admin/users/:id", async (c) => {
	...
	const id = c.req.param("id");
	const db = getDb(c.env);
	const { sessions, applications } = await import("./schema_BqQlk95h.mjs").then((n) => n.i);
	const { inArray } = await import("./drizzle-orm_BlbJG4Hn.mjs").then((n) => n.t);
	const employer = await db.select().from(employers).where(eq(employers.userId, id)).get();
	if (employer) {
		const jobIds = (await db.select({ id: jobs.id }).from(jobs).where(eq(jobs.employerId, employer.id)).all()).map((j) => j.id);
		if (jobIds.length > 0) {
...
//#endregion
//#region \0virtual:astro:page:src/pages/api/[...route]@_@ts
var page = () => ____route__exports;
//#endregion
export { page };
